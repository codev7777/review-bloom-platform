  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 className="text-5xl font-semibold mt-0 mb-0 text-center text-white  z-10 bg-[#f97316]">
          {title}
        </h3>
        <div className="text-center mt-16 mb-12 text-white text-3xl font-bold z-10 bg-[#2e3a48]">
          <div className="mb-2">
            <span className="text-6xl text-[#3d64f2]">${AnnuallyPrice}</span>
            <span className="text-3lg text-white"> /mo</span>
          </div>
          <div className="text-lg text-green-500 font-semibold z-10 bg-[#2e3a48]">
            (billed annually — save 20%)
          </div>
          <div className="mt-2 text-lg text-white opacity-70  z-10 bg-[#2e3a48]">
            or <span className="font-medium">${MonthlyPrice}/mo</span> billed
            monthly
          </div>
          {/* </div> */}
        </div>
        <ul className="space-y-3 mb-8 flex-grow ml-7 z-10 bg-[#2e3a48] mr-10">
          {features.map((feature, index) => (
            <motion.li
              key={index}
              className="flex items-start text-white text-xl z-10 bg-[#2e3a48]"
              initial={{ opacity: 0, x: -10 }}
              whileInView={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.4, delay: index * 0.1 }}
            >
              {feature.included ? (
                <Check className="w-6 h-6 flex-shrink-0 mt-0.5 text-green-500  z-10 bg-[#2e3a48]" />
              ) : (
                <X className="w-6 h-6 flex-shrink-0 mt-0.5 text-red-500 z-10 bg-[#2e3a48]" />
              )}
              <span className="ml-3 text-lg z-10 bg-[#2e3a48]">
                {feature.name}
              </span>
            </motion.li>
          ))}
        ...
        >
          <Button
            className={` w-full bg-[#fc880a] hover:bg-[#f97316] inline text-white px-10 py-5 xl:text-2xl text-2xl lg:text-xl z-10 bg-[#2e3a48]`}
            asChild
          >
